...
export const BottomNav: React.FC<BottomNavProps> = ({
  ...
  return (
    <nav
      ...
      className="lg:hidden fixed bottom-0 left-0 right-0 z-40 bg-white/95 dark:bg-[#12241F]/95 backdrop-blur-md border-t border-[#D9E0D4] dark:border-[#203830] shadow-[0_-4px_16px_rgba(0,0,0,0.06)] dark:shadow-[0_-4px_16px_rgba(0,0,0,0.3)] select-none print:hidden transition-all duration-200"
          ...
          return (
            <button
              ...
              className={`relative flex-1 flex flex-col items-center justify-center py-1 sm:py-1.5 px-1 rounded-xl transition-all duration-150 cursor-pointer group ${
                isActive
                  ? 'text-[#1D4137] dark:text-[#6EE7B7]'
                  : 'text-[#647169] dark:text-gray-400 hover:text-[#1D4137] dark:hover:text-gray-200'
              }`}
            >
              ...
              {isActive && (
                <motion.div
                  ...
                  className="absolute inset-0 bg-[#EFF2EA] dark:bg-[#18362D] rounded-xl -z-10 border border-[#D9E0D4]/80 dark:border-[#2D483F]"
              ...
              <div className="relative">
                <IconComponent
                  className={`w-5 h-5 transition-transform duration-150 ${
                    isActive ? 'scale-110 text-[#C9862E] dark:text-[#FBBF24]' : 'group-hover:scale-105'
                  }`}
                />

                {item.badge !== undefined && (
                  <span className="absolute -top-1.5 -right-2 min-w-[15px] h-[15px] px-1 rounded-full bg-[#C9862E] text-white text-[9px] font-bold font-mono flex items-center justify-center shadow-xs">
                    {item.badge}
                  </span>
                )}
              </div>
              ...
              <span
                className={`text-[10px] sm:text-[11px] mt-0.5 tracking-tight font-semibold leading-tight ${
                  isActive ? 'font-bold text-[#1D4137] dark:text-white' : ''
                }`}
              >
                {item.label}
              </span>
